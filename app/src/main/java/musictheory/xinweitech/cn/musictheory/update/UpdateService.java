...
public class UpdateService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent == null) {
            notifyUser(MusicApplication.getInstance().getResources().getString(R.string.download_erro), 0);
            stopSelf();
        } else {
    ...
    private void startDonwnLoad() {
        UpdateManager.getInstance().startDownload(apkUrl, localPath, new UpdateDownLoadListener() {
            ...
            public void onProgressChanged(int progress) {
                notifyUser(MusicApplication.getInstance().getResources().getString(R.string.download_is_in_progress), progress);
            }

            @Override
            public void onFinished(float fileSize) {
                notifyUser(MusicApplication.getInstance().getResources().getString(R.string.download_complete), 100);
                stopSelf();
            }
